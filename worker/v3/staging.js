...
function injectBridge(html,path){
  if(!isBridgeShell(path))return String(html);
  const tag='<script src="/__v3/production-editor-bridge.js"></script>';
  if(String(html).includes('/__v3/production-editor-bridge.js'))return html;
  if(/<head[^>]*>/i.test(html))return String(html).replace(/<head([^>]*)>/i,'<head$1>'+tag);
  return tag+html;
}
...
export async function proxyProduction(request,engine){
  ...
  if(ct.includes('text/html')){let html=await res.text();html=injectBridge(html,src.pathname);return new Response(html,{status:res.status,headers});}
