...
export default{async fetch(request,env){const url=new URL(request.url);try{
  if(url.pathname==='/health')return json({ok:true,service:'dinifaqih-engine-v3-lab',engine:ENGINE,phase:3,branch:'engine/v3-source-native-fidelity',baseline:BASELINE,browserBinding:Boolean(env.BROWSER),popupSystem:true});
