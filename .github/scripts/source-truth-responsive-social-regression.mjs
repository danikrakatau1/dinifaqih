...
const visibleEval=el=>{
  if(!el)return false;
  const s=getComputedStyle(el);
  return s.display!=='none' && s.visibility!=='hidden' && s.visibility!=='collapse' && Number.parseFloat(s.opacity||'1')>0 && !el.classList.contains('elementor-invisible');
};
...
async function inspectInvite(page,url,label,expectedGuest=''){
  ...
  await frame.waitForSelector('[data-id="43f80b1b"]',{timeout:45000});
  await frame.waitForSelector('[data-id="678f991a"]',{timeout:45000});
...
async function editorBridgeContract(page){
  ...
  await f.waitForSelector('[data-id="678f991a"]');await page.waitForTimeout(600);
