...
(()=>{
 ...
 async function putAppliedSnapshot(snap){const db=await openDB();await new Promise((res,rej)=>{const tx=db.transaction('snapshots','readwrite');tx.objectStore('snapshots').put(snap,appliedSnapshotIdbKey());tx.oncomplete=()=>res();tx.onerror=()=>rej(tx.error);tx.onabort=()=>rej(tx.error||new Error('IndexedDB transaction aborted'))});try{localStorage.setItem(appliedRefKey(),JSON.stringify({store:'indexeddb',db:'dini-anif-editor-v150',key:appliedSnapshotIdbKey(),revision:snap.revision,applied_at:snap.applied_at}))}catch{};try{localStorage.removeItem(scopedStorageKey('diniAnifNativeApplied:v229'))}catch{};try{sessionStorage.removeItem(scopedStorageKey('diniAnifCleanPreviewApplied:v229'))}catch{}}
 async function getAppliedSnapshot(){try{const db=await openDB();const snap=await new Promise((res,rej)=>{const tx=db.transaction('snapshots');const q=tx.objectStore('snapshots').get(appliedSnapshotIdbKey());q.onsuccess=()=>res(q.result||null);q.onerror=()=>rej(q.error)});if(snap)return snap}catch(e){console.warn('IndexedDB applied snapshot read failed',e)}try{return JSON.parse(localStorage.getItem(scopedStorageKey('diniAnifNativeApplied:v229'))||'null')}catch{return null}}
 async function deleteAppliedSnapshot(){try{const db=await openDB();await new Promise((res,rej)=>{const tx=db.transaction('snapshots','readwrite');tx.objectStore('snapshots').delete(appliedSnapshotIdbKey());tx.oncomplete=()=>res();tx.onerror=()=>rej(tx.error)})}catch{}try{localStorage.removeItem(scopedStorageKey('diniAnifNativeApplied:v229'));localStorage.removeItem(appliedRefKey())}catch{}try{sessionStorage.removeItem(scopedStorageKey('diniAnifCleanPreviewApplied:v229'))}catch{}}
 ...
 async function setCurrentRecordContextStrict(){
 ...
 async function snapshotCurrentEditorForPackage(){
   ...
   return {version:'2.29.0',template_context:templateContextId(),revision,schema:deep(native.schema),manifest:deep(native.manifest||{}),values:deep(native.values||{}),transforms:deep(transforms||{}),html:rebuildHtml({forExport:true}),assets,applied_at:new Date().toISOString()};
 }
 ...
 resetBtn.onclick=async()=>{if(!confirm('Reset hanya state V2.29 untuk template ini?'))return;localStorage.removeItem(DRAFT_KEY);localStorage.removeItem(templateRecoveryKey());localStorage.removeItem(pendingB2Key());sessionStorage.removeItem(handoffKey());localStorage.removeItem(handoffKey());try{if(typeof window.name==='string'&&window.name.startsWith(handoffWindowPrefix()))window.name=''}catch{}await deleteAppliedSnapshot();location.reload()};
 ...
 // V2.29: side-by-side recovery never mutates legacy editor storage.
