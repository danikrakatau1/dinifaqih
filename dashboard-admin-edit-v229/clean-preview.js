(async()=>{
  const frame=document.getElementById('cleanFrame'),empty=document.getElementById('empty');let snap=null;const params=new URLSearchParams(location.search);const context=String(params.get('template')||'standalone');const snapKey=`native-applied:v229:${context}`;
  ...
  try{const db=await openDB();snap=await new Promise((res,rej)=>{const tx=db.transaction('snapshots');const q=tx.objectStore('snapshots').get(snapKey);q.onsuccess=()=>res(q.result||null);q.onerror=()=>rej(q.error)})}catch(e){console.warn('Applied snapshot IndexedDB read:',e)}
  if(!snap?.html||String(snap.template_context||context)!==context){frame.hidden=true;empty.hidden=false;empty.textContent='Belum ada APPLY V2.29 untuk template UUID ini.';return}empty.hidden=true;frame.hidden=false;
