(()=>{
  ...
  // V1.16.1 parity lock: this mirrors Editor applyTransform() for backgrounds.
  // Slideshow zoom belongs on the visual slide child, not on the section/host itself.
  function applyTransform(n,f){
    const t=transforms[f.id];if(!n||!t)return;
    const x=Number.isFinite(Number(t.x))?Number(t.x):50;
    const y=Number.isFinite(Number(t.y))?Number(t.y):50;
    const scale=Math.max(.25,Math.min(3,Number(t.scale)||1));
    const rotate=Number(t.rotate)||0;
    ...
    n.style.setProperty('--native-bg-position',`${x}% ${y}%`);
    const pic=n.matches?.('[data-native-slide-bg]')?n:n.querySelector?.('[data-native-slide-bg]');
    if(pic){
      pic.style.backgroundPosition=`${x}% ${y}%`;
      pic.style.transformOrigin=`${x}% ${y}%`;
      pic.style.transform=`scale(${scale}) rotate(${rotate}deg)`;
      pic.style.backgroundSize=fit==='fill'?'100% 100%':fit;
      return;
    }
    n.style.backgroundPosition=`${x}% ${y}%`;
    if(Math.abs(scale-1)<.001){
      n.style.backgroundSize=fit==='fill'?'100% 100%':fit;
    }else if(fit==='fill'){
      const pct=(scale*100).toFixed(2).replace(/\.00$/,'');
      n.style.backgroundSize=`${pct}% ${pct}%`;
    }else{
      const pct=(scale*100).toFixed(2).replace(/\.00$/,'');
      n.style.backgroundSize=`${pct}% auto`;
