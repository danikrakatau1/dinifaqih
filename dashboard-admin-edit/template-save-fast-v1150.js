(()=>{
  'use strict';
  if(window.__DINI_TEMPLATE_SAVE_FAST_1150__)return;
  window.__DINI_TEMPLATE_SAVE_FAST_1150__=true;
  ...
  async function readApplied(){const id=urlRecordId();if(id){const scoped=await readSnapKey(scopedKey(id));if(scoped)return scoped;throw new Error('Snapshot APPLY khusus template ini belum ada. Klik APPLY sekali lagi sebelum menyimpan template.')}return readSnapKey(GLOBAL_KEY)}
  ...
  function canonicalAssetBase(html,manifest={}){
    try{const doc=new DOMParser().parseFromString(String(html||''),'text/html');const raw=String(doc.querySelector('base')?.getAttribute('href')||'').trim();if(raw){const resolved=new URL(raw,manifest.source_url||location.href).href;const d=dirname(resolved);if(d)return d}}catch{}
  ...
  async function saveAppliedAsTemplate(){
    ...
    saving=true;if(btn)btn.disabled=true;const t=toast(isFetchTool?'Menyiapkan template baru dari Fetch Editor…':'Membaca snapshot template terpilih…','loading',isFetchTool?'Simpan Template Baru':'Update Template');
    try{
      ...
      const snap=await withTimeout(readApplied(),12000,'Snapshot APPLY');if(!snap?.schema||!Array.isArray(snap.schema.fields)||!snap.html)throw new Error('Snapshot APPLY belum tersedia/valid. Klik APPLY sekali lagi.');
      ...
      const row=isFetchTool?null:await currentRow();
      if(!isFetchTool&&!row)throw new Error('Template yang dipilih tidak ditemukan. Kembali ke Template Library lalu klik Edit pada template yang benar.');
      ...
      const manifest={...manifestBase,editor_version:'2.32.0',renderer_version:'template-contract-1.15.0',revision,revision_id:revision,template_id:ident.id,record_id:ident.id,editor_snapshot_url:snapshotUrl,source_of_truth:isFetchTool?'fetch-editor-applied-snapshot':'template-editor-applied-snapshot',snapshot_scope_version:'1.15.0',asset_base:assetBase,saved_at:new Date().toISOString(),artifact_prefix:base,...(pendingB2?.object_key?{package_storage:pendingB2}:{})};
      const cloudSnap={...deep(snap),version:'2.32.0',template_id:ident.id,record_id:ident.id,snapshot_scope:ident.id,snapshot_scope_version:'1.15.0',revision,html:finalHtml,baseHtml:finalHtml,values,schema,manifest,assets:[],saved_at:new Date().toISOString()};
      ...
      const status=row?(row.status||(row.is_active?'active':'draft')):'draft';
      ...
      if(small)small.textContent=isFetchTool?'Mendaftarkan template baru…':'Memperbarui UUID template yang dipilih…';
      let result;if(row)result=await withTimeout(sb.from('templates').update(payload).eq('id',ident.id).select('*').single(),15000,'Update template terpilih');else result=await withTimeout(sb.from('templates').insert({id:ident.id,...payload}).select('*').single(),15000,'Insert template baru');if(result.error)throw result.error;
      ...
      const localScoped={...deep(snap),template_id:ident.id,record_id:ident.id,snapshot_scope:ident.id,snapshot_scope_version:'1.15.0'};await writeSnapKey(scopedKey(ident.id),localScoped);await writeSnapKey(GLOBAL_KEY,localScoped);
      finishToast(t,isFetchTool?`${verify.data.name} dibuat sebagai template baru dari Fetch Editor.`:`${verify.data.name} diperbarui pada UUID yang sama.`,'success',isFetchTool?'Template baru tersimpan':'Template diperbarui');
      await sleep(650);location.href='/dashboard-admin-template?refresh='+Date.now();
    }catch(err){console.error('TEMPLATE_SAVE_FAST_V1150',err);finishToast(t,err?.message||String(err),'error','Simpan Template gagal');if(btn)btn.disabled=false}finally{saving=false}
  }
  ...
  document.documentElement.dataset.templateSave='v1.15.0';
})();
