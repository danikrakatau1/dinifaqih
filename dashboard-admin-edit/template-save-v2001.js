(()=>{
  ...
  async function save(){
    ...
    try{
      ...
      const liveStreamContract={version:1,contract:'dini-live-stream-v1',count:new Set(liveFields.map(f=>f.section_id).filter(Boolean)).size,section_ids:[...new Set(liveFields.map(f=>f.section_id).filter(Boolean))],enabled_field_ids:liveFields.filter(f=>f.kind==='live-enabled').map(f=>f.id),url_field_ids:liveFields.filter(f=>f.kind==='live-url').map(f=>f.id),source_player_policy:'removed',embed_policy:'youtube-url-only',external_url_policy:'button-only'};
      ...
      if(manifestBase.runtime_manifest){
        ...
        manifestBase.runtime_manifest.live_stream_contract=liveStreamContract;
        ...
        if(!manifestBase.runtime_manifest.capabilities.includes('live-stream'))manifestBase.runtime_manifest.capabilities.push('live-stream');
