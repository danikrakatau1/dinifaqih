...
(()=>{
 ...
 const LEGACY_GLOBAL_DRAFT_KEY='diniAnifNativeDraft-v169';
 ...
 function ensureRuntimeCssBackgroundFields(doc){
   ...
   if(changed){native.schema.field_count=native.schema.fields.length;try{localStorage.setItem(draftKey(),JSON.stringify({schema:native.schema,baseHtml:native.baseHtml,manifest:native.manifest,values:native.values}))}catch{}}
   return changed
 }
 let recoveryTimer=0;
 ...
 function schedulePreview(){clearTimeout(previewTimer);previewTimer=setTimeout(()=>renderPreview(true),300)}
 ...
 function renderPreview(force=false){if(!native){frame.removeAttribute('srcdoc');frame.src='./invitation.html?editor=1';return}if(!force&&frame.srcdoc&&liveDoc())return;const exactHandoff=editorParams.get('handoff')==='1';const html=exactHandoff?native.baseHtml:rebuildHtml();frame.onload=()=>{installFrameBridge();if(editorParams.get('handoff')==='1'){setTimeout(()=>{try{installFrameBridge();if(selectedFieldId)highlightSelection()}catch{}},80);return}setTimeout(()=>{try{const d=liveDoc(),w=d?.defaultView;if(!d?.body||!w)return;d.documentElement.style.setProperty('visibility','visible','important');d.documentElement.style.setProperty('opacity','1','important');d.body.style.setProperty('visibility','visible','important');d.body.style.setProperty('opacity','1','important');d.querySelectorAll('.e-con.e-parent').forEach(el=>el.classList.add('e-lazyloaded'));const cover=d.querySelector('#cover,.cover')||d.querySelector('.elementor-top-section,body>section');if(cover){cover.hidden=false;cover.removeAttribute('aria-hidden');cover.style.setProperty('display','block','important');cover.style.setProperty('visibility','visible','important');cover.style.setProperty('opacity','1','important');cover.querySelectorAll('.elementor-invisible,[data-native-reveal]').forEach(el=>{el.classList.remove('elementor-invisible');el.classList.add('native-visible');el.style.setProperty('visibility','visible','important');el.style.setProperty('opacity','1','important')});cover.querySelectorAll('img').forEach(el=>{el.style.setProperty('visibility','visible','important');el.style.setProperty('opacity','1','important')})}const open=d.querySelector('[data-native-open],#tombolbuka,.tombolbuka');if(open){open.classList.remove('elementor-invisible');open.classList.add('native-visible','btnVisibleAfterLoad');open.style.setProperty('display','block','important');open.style.setProperty('margin-left','auto','important');open.style.setProperty('margin-right','auto','important');open.style.setProperty('visibility','visible','important');open.style.setProperty('opacity','1','important');open.style.setProperty('transform','none','important');open.style.setProperty('pointer-events','auto','important')}d.documentElement.setAttribute('data-dini-editor-live-stabilized','v2273b');installFrameBridge()}catch(err){console.warn('EDITOR_LIVE_STABILIZER',err)}},60);setTimeout(()=>{try{const d=liveDoc(),b=d?.body;if(!b)return;const els=[...b.querySelectorAll('*')].filter(el=>{const cs=d.defaultView.getComputedStyle(el),r=el.getBoundingClientRect();return cs.display!=='none'&&cs.visibility!=='hidden'&&Number(cs.opacity||1)>0&&r.width>2&&r.height>2});if(!els.length){const handoffMode=editorParams.get('handoff')==='1';if(handoffMode){console.info('Exact-template handoff: generic blank fallback disabled; preserving selected template source');setTimeout(()=>{try{installFrameBridge();if(selectedFieldId)highlightSelection()}catch{}},1400);return}console.warn('Editor preview blank guard triggered');frame.removeAttribute('srcdoc');frame.src='./invitation.html?editor=1&blankguard=1';editorToast('Draft preview lama tidak kompatibel. Import ulang Rebuild ZIP terbaru.','info','Preview dipulihkan')}}catch(e){console.warn('blank guard',e)}},700)};frame.removeAttribute('src');frame.srcdoc='';requestAnimationFrame(()=>{frame.srcdoc=html})}
 ...
 function uploadFor(id,accept){const inp=document.createElement('input');inp.type='file';inp.accept=accept||'*/*';inp.onchange=()=>{const f=inp.files?.[0];if(!f)return;pushHistory();const name=f.name.replace(/[^a-zA-Z0-9._-]+/g,'-');generatedAssets.set(id,{blob:f,name,type:f.type||'application/octet-stream',path:'assets/generated/'+id+'-'+name,source:'upload'});setDirty('Perubahan asset belum di-APPLY');if(!liveApply(id))schedulePreview();renderNativeEditor();selectField(id,false);editorToast(`${f.name} siap dan akan ikut Production ZIP.`,'success','Upload sukses')};inp.click()}
 function deleteFieldAsset(id){pushHistory();generatedAssets.delete(id);native.values[id]='';setDirty();if(!liveApply(id))schedulePreview();renderNativeEditor();renderInspector()}
 ...
 function appliedRevisionIdbKey(revision){return `${appliedSnapshotIdbKey()}:${String(revision||'')}`}
 ...
 async function uploadPackageToB2(file){
   ...
   try{
     ...
     if(currentTemplateRecord&&!currentTemplateRecord.is_active){
       ...
       try{localStorage.setItem(pendingB2Key(),JSON.stringify(pendingB2Upload))}catch{}
 ...
 async function saveCurrentDraft(options={}){
   ...
   try{
     ...
     try{localStorage.setItem(draftKey(),JSON.stringify({schema:native.schema,baseHtml:native.baseHtml,manifest:native.manifest,values:native.values,transforms:deep(transforms)}))}catch{};saveRecovery()
     ...
     try{localStorage.removeItem(pendingB2Key())}catch{}
 ...
 async function loadTemplateFromLibrary(recordOnly=false){
   ...
   try{
     ...
     if(recordOnly){finishEditorToast(t,`${data.name} dipilih. Menunggu Source Graph khusus template ini…`,'success','Template terkunci');return true}
 ...
 function cleanPreviewUrl(snap,{download=false}={}){const q=new URLSearchParams({template:templateContextId(),rev:String(snap?.revision||''),v:'2282'});if(download)q.set('download','1');return './clean-preview.html?'+q.toString()}
 ...
 async function bootEditor(){
   ...
   if(!fromLibrary&&!loadSnapshotHandoff()){
     try{
       ...
       const d=scoped?.schema&&scoped?.baseHtml?scoped:JSON.parse(localStorage.getItem(draftKey())||'null');
