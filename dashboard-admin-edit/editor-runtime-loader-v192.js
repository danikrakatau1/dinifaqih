(async()=>{
  ...
  try{
    const r=await fetch('./editor.js?v=source-runtime-192',{cache:'no-store'});
    if(!r.ok)throw new Error('editor.js HTTP '+r.status);
    let src=await r.text();

    // Keep all original editor behavior, but make media ownership visual-first instead of DOM-first.
    ...
    document.documentElement.dataset.editorRuntime='v1.9.2';
    const d=document.getElementById('dirtyState');if(d&&/Loading Editor/i.test(d.textContent||''))d.textContent='EDITOR V1.9.2 READY';
  }catch(err){
    console.error('EDITOR_RUNTIME_LOADER_V192',err);
    const d=document.getElementById('dirtyState');if(d)d.textContent='EDITOR V1.9.2 LOAD ERROR: '+(err.message||err);
  }
})();
