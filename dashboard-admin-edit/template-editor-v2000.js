(async()=>{
  ...
  function setBg(n,value){if(!n)return;const css=value?`url("${String(value).replaceAll('"','%22')}")`:'none';n.style.setProperty('background-image',css,'important');if(n.hasAttribute?.('data-thumbnail'))n.setAttribute('data-thumbnail',value||'');syncMediaClickTarget(n,value)}
  ...
  function syncElementorActionHash(a,value){const raw=String(a?.getAttribute?.('data-e-action-hash')||'').trim();if(!raw||!value)return false;try{const d=decodeURIComponent(raw.replace(/^#/,'')),p='elementor-action:';if(!d.startsWith(p))return false;const q=new URLSearchParams(d.slice(p.length)),b=q.get('settings');if(!b)return false;const cfg=JSON.parse(atob(b));if(!cfg||typeof cfg!=='object'||String(cfg.url||'')===String(value))return false;cfg.url=String(value);q.set('settings',btoa(JSON.stringify(cfg)));a.setAttribute('data-e-action-hash','#'+encodeURIComponent(p+q.toString()));a.setAttribute('data-dini-lightbox-action-authority','display-media');return true}catch{return false}}
  function syncMediaClickTarget(node,value){if(!node)return false;const a=node.closest?.('a[href]');if(!a||!isImageLightboxAnchor(a))return false;let changed=false;if(value){if(a.getAttribute('href')!==String(value)){a.setAttribute('href',String(value));changed=true}if(syncElementorActionHash(a,String(value)))changed=true;a.setAttribute('data-dini-image-authority','display-media')}else a.removeAttribute('href');return changed}
  function galleryMediaUrl(a){if(!a)return'';const n=a.querySelector?.('[data-thumbnail],.e-gallery-image,[style*="background-image"],img[src],img[data-src]');if(!n)return'';const direct=String(n.getAttribute?.('data-thumbnail')||n.getAttribute?.('src')||n.getAttribute?.('data-src')||'').trim();if(direct)return direct;const m=String(n.style?.getPropertyValue?.('background-image')||n.getAttribute?.('style')||'').match(/url\(\s*(["']?)(.*?)\1\s*\)/i);return String(m?.[2]||'').trim()}
  function reconcileImageClickTargets(doc){let changed=0;for(const a of doc?.querySelectorAll?.('a[href]')||[]){if(!isImageLightboxAnchor(a))continue;const src=galleryMediaUrl(a);if(!src)continue;if(a.getAttribute('href')!==src){a.setAttribute('href',src);changed++}if(syncElementorActionHash(a,src))changed++;a.setAttribute('data-dini-image-authority','display-media')}return changed}
