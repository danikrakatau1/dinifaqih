(async()=>{
  ...
  try{
    const r=await fetch('./editor.js?v=source-runtime-194',{cache:'no-store'});
...

    {
      ...
      if(!src.includes(old))throw new Error('Anchor click-media V1.9.4 tidak ditemukan');
      src=src.replace(old,replacement);
    }
    ...
    (0,eval)(src+'\n//# sourceURL=editor-runtime-v194.js');
    ...
    document.documentElement.dataset.editorRuntime='v1.9.4';
    const d=document.getElementById('dirtyState');if(d&&/Loading Editor/i.test(d.textContent||''))d.textContent='EDITOR V1.9.4 READY';
  }catch(err){
    console.error('EDITOR_RUNTIME_LOADER_V194',err);
    const d=document.getElementById('dirtyState');if(d)d.textContent='EDITOR V1.9.4 LOAD ERROR: '+(err.message||err);
  }
})();
