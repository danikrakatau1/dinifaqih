(async()=>{
  ...
  try{
    const r=await fetch('./editor.js?v=source-runtime-193',{cache:'no-store'});
...

    {
      ...
      if(!src.includes(old))throw new Error('Anchor click-media V1.9.3 tidak ditemukan');
      src=src.replace(old,replacement);
    }
    ...
    document.documentElement.dataset.editorRuntime='v1.9.3';
    const d=document.getElementById('dirtyState');if(d&&/Loading Editor/i.test(d.textContent||''))d.textContent='EDITOR V1.9.3 READY';
  }catch(err){
    console.error('EDITOR_RUNTIME_LOADER_V193',err);
    const d=document.getElementById('dirtyState');if(d)d.textContent='EDITOR V1.9.3 LOAD ERROR: '+(err.message||err);
  }
})();
