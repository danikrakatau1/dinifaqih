(() => {
  ...
  function ownedNodes(doc,f){
    ...
    // V1.10.1: css-overlay/portrait must NEVER paint both the visual overlay
    // and its Elementor host. That was the source of the rectangular duplicate.
    if(f.media_role==='css-overlay'){
      ...
      if(f.source_element_id){
        ...
        if(host){
          overlay=host.querySelector(':scope > .elementor-widget-wrap > .elementor-background-overlay,:scope > .elementor-element-populated > .elementor-background-overlay,:scope > .elementor-background-overlay')||host.querySelector('.elementor-background-overlay');
        }
      }
      for(const n of direct){
        if(n===host)continue;
        if(isOverlayNode(n)||n===overlay)add(n);
      }
      if(overlay)add(overlay);
      // Some source-native overlays are synthetic nodes marked directly with the field id
      // but are not Elementor's standard .elementor-background-overlay.
      if(!out.length){
        for(const n of direct){if(n!==host)add(n)}
      }
      // Last-resort fallback only when no separate overlay exists at all.
    ...
    if(f.kind==='background'){
      if(f.source_element_id)add(doc.querySelector(`[data-id="${esc(f.source_element_id)}"]`));
  ...
  }).catch(e=>console.error('CLEAN_MEDIA_AUTHORITY_V1101',e));
