(async()=>{
  ...
  try{
...

    {
      ...
      const replacement=" function deleteFieldAsset(id){const f=fieldById(id);const ids=(typeof linkedMediaAliasIds==='function'&&f&&['image','background'].includes(f.kind))?linkedMediaAliasIds(id):[id];const active=ids.some(x=>generatedAssets.has(x)||String(native.values[x]??fieldById(x)?.value??'')!=='');if(!active)return;pushHistory();for(const aid of ids){generatedAssets.delete(aid);native.values[aid]='';liveApply(aid)}setDirty('Asset dihapus · perubahan belum di-APPLY');renderNativeEditor();renderInspector()}";
