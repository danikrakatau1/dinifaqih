(()=>{
  'use strict';
  if(window.__DINI_TEMPLATE_EDIT_CLOUD_BOOTSTRAP_1150__)return;
  window.__DINI_TEMPLATE_EDIT_CLOUD_BOOTSTRAP_1150__=true;
  ...
  const frame=document.getElementById('previewFrame'),dirty=document.getElementById('dirtyState');
  ...
  const embeddedBase=(html,manifest={},fallback='')=>{
    ...
    const fb=dirname(fallback);return fb||location.origin+'/';
  };
  const inject=(html,payload)=>{
    const json=JSON.stringify(payload).replace(/</g,'\\u003c').replace(/-->/g,'--\\>');
    const block='<script type="application/json" id="diniSnapshotAuthorityData">'+json+'<\/script><script src="/dashboard-admin-template/snapshot-authority-v1150.js?v=1150"><\/script>';
  ...
  async function hydrate(){
    ...
    try{
      const rowRes=await sb.from('templates').select('id,name,slug,source_path,manifest_json,updated_at').eq('id',recordId).maybeSingle();
      if(rowRes.error)throw rowRes.error;const row=rowRes.data;if(!row||String(row.id)!==String(recordId))throw new Error('UUID template yang dipilih tidak cocok.');
      ...
      const u=new URL(snapUrl,location.href);u.searchParams.set('_edit',String(row.updated_at||Date.now()));
      const r=await fetch(u.href,{cache:'no-store'});if(!r.ok)throw new Error('Editor Snapshot HTTP '+r.status);const snap=await r.json();
      ...
      frame.srcdoc=html;done=true;
      document.documentElement.dataset.templateEditAuthority='v1.15.0';
      if(dirty&&/DRAFT CLOUD LOADED/i.test(dirty.textContent||''))dirty.textContent=`DRAFT CLOUD LOADED ✓ · UUID ${recordId.slice(0,8)} · EXACT SNAPSHOT V1.15`;
    }catch(err){console.error('TEMPLATE_EDIT_CLOUD_BOOTSTRAP_V1150',err);window.editorToast?.(err.message||String(err),'error','Template Edit hydrate gagal')}
    finally{running=false}
  }

  let tries=0;const timer=setInterval(()=>{
    tries++;const state=String(dirty?.textContent||'');
    if(/DRAFT CLOUD LOADED/i.test(state)){clearInterval(timer);setTimeout(hydrate,40)}
