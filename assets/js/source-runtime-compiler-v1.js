...
const open=ev=>guard('open-invitation',()=>{if(opened)return;opened=true;if(ev){ev.preventDefault();ev.stopPropagation()}(plan.on_open||[]).forEach(run);startObserver();if(cover&&!plan.on_open?.some?.(x=>x.selector==='#cover'&&(x.type==='animate-style'||x.type==='hide'||x.type==='fade-out'))){cover.style.transition='opacity .45s ease';cover.style.opacity='0';setTimeout(()=>guard('cover-dismiss',()=>{cover.style.display='none';cover.setAttribute('aria-hidden','true')}),450)}});
