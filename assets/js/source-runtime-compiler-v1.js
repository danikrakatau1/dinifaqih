...
const startObserver=()=>guard('intersection-observer',()=>{const nodes=[...document.querySelectorAll('[data-native-reveal]')].filter(el=>!cover?.contains(el));if(!('IntersectionObserver' in window)){nodes.forEach(reveal);return}const io=new IntersectionObserver(es=>es.forEach(e=>guard('intersection-entry',()=>{if(e.isIntersecting){reveal(e.target);io.unobserve(e.target)}})),{threshold:.12,rootMargin:'0px 0px -6%'});nodes.forEach(el=>io.observe(el))});
...
const open=ev=>guard('open-invitation',()=>{if(opened)return;opened=true;if(ev){ev.preventDefault();ev.stopPropagation()}openPlan.forEach(run);startObserver();if(cover&&!openPlan.some(x=>x.selector==='#cover'&&(x.type==='animate-style'||x.type==='hide'||x.type==='fade-out'))){cover.style.transition='opacity .45s ease';cover.style.opacity='0';setTimeout(()=>guard('cover-dismiss',()=>{cover.style.display='none';cover.setAttribute('aria-hidden','true')}),450)}});
...
document.documentElement.setAttribute('data-dini-source-runtime','${VERSION}');document.documentElement.setAttribute('data-dini-fault-isolation','1');
})();`}
  ...
  console.info('[DINI SOURCE RUNTIME] V'+VERSION+' aktif — safe lifecycle + per-command fault isolation, no arbitrary source JS.');
})(window);