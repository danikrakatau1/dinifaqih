(()=>{
  ...
  function prepareHtml(pkg,row,guest){
    ...
    const blocks=[];
    if(pkg.source_truth){
      blocks.push('<script>document.documentElement.dataset.diniSourceTruthPublic="1";document.documentElement.dataset.diniPublicCompatibility="source-truth-exact";</script>');
    }else{
      if(pkg.authority){
