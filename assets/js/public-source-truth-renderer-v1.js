  ...
  function prepareHtml(pkg,row,guest){
    ...
    if(pkg.source_truth){
      ...
      if(galleryPerf.count){
        ...
        blocks.push('<script src="'+localAsset('/assets/js/gallery-love-shared-layer-v1.js?v=280')+'"></script>');
      }
    }else{
      if(pkg.authority){
    ...
    blocks.push('<script type="application/json" id="diniPostOpenWelcomeConfig">'+safeJson({
      mode:guest?'guest':'public',
      guestName:String(guest?.name||''),
      delayMs:1500,
      holdMs:2500
    })+'</script>');
    blocks.push('<script src="'+localAsset('/assets/js/post-open-welcome-v1.js?v=310')+'"></script>');
