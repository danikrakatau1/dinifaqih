(function(g){
  ...
  function compileEvents(doc){
    ...
    headings.forEach((titleNode,index)=>{
      ...
      const container=eventContainerFor(titleNode);
  ...
  function countdownDisplays(doc){
    ...
    return roots.map((root,index)=>{
      const slot=(name,aliases=[])=>{
        ...
        if(!label){
          const candidates=[...root.querySelectorAll('span,small,p,div')].map(x=>clean(x.textContent)).filter(t=>t&&t.length<32);
          label=candidates.find(t=>aliases.some(a=>new RegExp('^'+a+'$','i').test(t)))||'';
        }
